...
/*
const links = document.querySelectorAll('.page__link');// отримуємо колекцію всіх тегів з класом page__link

links.forEach(link => {
    link.addEventListener("click", linkActionTwo);
})
*/
// Насправді не дуже добрий варіант , адже багато подій одночасно відслідковуються і тому навантажуюуть систему
//? Дізнатися більше про стрілкові функції

//Делегування подій
document.addEventListener("click", action); // додаю прослховування до всього документа (функція спрацює при кліку у будь-якому місці документа)
// пам'ятаємо про синтакси та запис функції без дужок у конструкції addEventListner('nameEvent',func)
function linkActionThree(event) {
    const targetItem = event.target; // константа отримає об'єкт по якому клікне мишею юзер
    if (targetItem.classList.contains('.link'))// перевіряємо , чи є клас у вибраного об'єкта
        //! не спрацює якщо всередині об'кту з класом link буде інший об'єкт з іншим класом
        console.log('it`s link');
}
//! суперсила closest
function action(event) {
    const targetItem = event.target; // константа отримає об'єкт по якому клікне мишею юзер
    if (targetItem.closest('.link')) {// перевіряємо , чи є клас у вибраного об'єкта чи його батьківського об'єкта
        // потрібний код
        console.log('it`s link or link has my parent');
        event.preventDefault(); //? вияснити чи потрібен тут preventDefault() чи це для демонстрації на уроці, аби було видно консольку
    }

    if (targetItem.closest('.button')) { // перевіряємо , чи є клас у вибраного об'єкта чи його батьківського об'єкта та додаємо нові дії 
        // код
    }

    if (targetItem.closest('.button2')) { // перевіряємо , чи є клас у вибраного об'єкта чи його батьківського об'єкта та додаємо нові дії 
        // код
    }
    //...
}
